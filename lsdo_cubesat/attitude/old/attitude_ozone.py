...
class Attitude(Model):
    ...
    def define(self):
        ...
        # p = Omega**2
        do0_dt = csdl.expand(
            K[0], n) * (omega1 * omega2 - 3 * Omega**2 * C01 * C02 + t0)
        do1_dt = csdl.expand(
            K[1], n) * (omega2 * omega0 - 3 * Omega**2 * C02 * C00 + t1)
        do2_dt = csdl.expand(
            K[2], n) * (omega0 * omega1 - 3 * Omega**2 * C00 * C01 + t2)
...
class P(ODEProblem):
    def setup(self):
        ...
        self.add_parameter('t0', dynamic=True, shape=(nt, 1))
        self.add_parameter('t1', dynamic=True, shape=(nt, 1))
        self.add_parameter('t2', dynamic=True, shape=(nt, 1))
        self.add_parameter('Omega', dynamic=True, shape=(nt, 1))
...
class RunModel(Model):
    ...
    def define(self):
        ...
        self.create_input('omega0_0', val=0.1)
        self.create_input('omega1_0', val=0.1)
        self.create_input('omega2_0', val=1.1)
        self.create_input('C00_0', val=C[0, 0])
        self.create_input('C01_0', val=C[0, 1])
        self.create_input('C02_0', val=C[0, 2])
        self.create_input('C20_0', val=C[1, 0])
        self.create_input('C21_0', val=C[1, 1])
        self.create_input('C22_0', val=C[1, 2])
        self.create_input('C10_0', val=C[2, 0])
        self.create_input('C11_0', val=C[2, 1])
        self.create_input('C12_0', val=C[2, 2])

        # Add to csdl model which are fed into ODE Model
        t0 = self.create_input('t0', shape=n, val=0)
        t1 = self.create_input('t1', shape=n, val=0)
        t2 = self.create_input('t2', shape=n, val=0)
        Omega = self.create_input('Omega', shape=n, val=1)
        K = self.create_input('K', val=K)
...
K3 = -(K1 + K2) / (1 + K1 * K2)
K = [K1, K2, K3]
sim = Simulator(RunModel(num_times=nt, step_size=0.01, K=K), mode='rev')
if nt < 15:
    ...
    nutation = 180 / np.pi * np.arccos(sim['C22'])
