...
class BitRateComp(ExplicitComponent):
    ...
    def setup(self):
        ...
        # Inputs
        self.add_input('P_comm', np.zeros((1, num_times)), units='W',
                       desc='Communication power over time')

        self.add_input('gain', np.zeros((1, num_times)), units=None,
                       desc='Transmitter gain over time')

        self.add_input('GSdist', np.zeros((1, num_times)), units='km',
                       desc='Distance from ground station to satellite over time')

        self.add_input('CommLOS', np.zeros((1, num_times)), units=None,
                       desc='Satellite to ground station line of sight over time')

        # Outputs
        self.add_output('Download_rate', np.zeros((1, num_times)))
    ...
    def compute(self, inputs, outputs):
        ...
        for i in range(0, num_times):
            ...
            Dr[0, i] = self.alpha * P_comm[0,i] * gain[0,i] * CommLOS[0,i] / S2 ** 2
    ...
    def compute_partials(self, inputs, partials):
        ...
        for i in range(0, num_times):
            ...
            self.dD_dP[i] = self.alpha * gain[0,i] * CommLOS[0,i] / S2 ** 2
            self.dD_dGt[i] = self.alpha * P_comm[0,i] * CommLOS[0,i] / S2 ** 2
            self.dD_dS[i] = -2.0 * 1e3 * self.alpha * P_comm[0,i] * gain[0,i] * CommLOS[0,i] / S2 ** 3
            self.dD_dLOS[i] = self.alpha * P_comm[0,i] * gain[0,i] / S2 ** 2
...
if __name__ == '__main__':
    ...
    comp.add_output('CommLOS', val=np.random.random((1, n)))
