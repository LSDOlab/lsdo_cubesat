...
from lsdo_cubesat.utils.bspline_comp import BsplineComp
...
class GSGroup(Group):
    ...
    def setup(self):
        ...
        self.add('P_comm_comp', comp, promotes=['*'])

        comp = BitRate(num_times=num_times)
        self.add('Download_rate', comp, promotes=['*'])

        comp = DataDownloadComp(
            num_times=num_times,
            step_size=step_size,
        )
        self.add('Data_download_rk4_comp', comp, promotes=['*'])

        comp = ExecComp(
            'total_data_downloaded= Data[-1] - Data[0]',
            Data=np.empty(num_times),
        )
        self.add('total_data_downloaded_comp', comp, promotes=['*'])
