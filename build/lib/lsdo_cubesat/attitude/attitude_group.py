...
class AttitudeGroup(Group):
    ...
    def setup(self):
        ...
        for var_name in ['roll', 'pitch']:
            ...
            self.add_subsystem('{}_comp'.format(var_name), comp, promotes=['*'])
        ...
        for var_name in [
            ...
            self.add_subsystem('d{}_comp'.format(var_name), comp, promotes=['*'])
        ...
        for var_name in [
            ...
            comp.add_constraint('{}_rate'.format(var_name), lower=-10. * rad_deg, upper=10. * rad_deg, linear=True)
            self.add_subsystem('{}_rate_comp'.format(var_name), comp, promotes=['*'])